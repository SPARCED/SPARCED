...
class drs_dict():
    ...
    def term_lins(self,g1cxs): #generate terminal lineages for groups of g1 cells
        ...
        for term_lin in term_lins:
            ...
            traj_lin_parp = xout_lin[:,list(species_all).index('PARP')]
            traj_lin_cparp = xout_lin[:,list(species_all).index('cPARP')]
...
#%% figure 2 - gen 0 vs gen 1

# load files

dir1 = os.path.join(wd,'output','cellpop_drs_demo')
dir0 = os.path.join(wd,'output','cellpop_drs_demo_g0')

with open(os.path.join(dir1,'trame_EC_0.0','output_g1.pkl'),'rb') as f1:
    demo_g1 = pickle.load(f1)
    

with open(os.path.join(dir0,'results_g0.pkl'),'rb') as f0:
    demo_g0 = pickle.load(f0)
...
for g0c in range(len(demo_g0.keys())):
    
    tout = np.linspace(0,len(demo_g0[str(g0c+1)]['xoutS'])*30,len(demo_g0[str(g0c+1)]['xoutS']))
    xout = demo_g0[str(g0c+1)]['xoutS']
    
    if max(xout)>35:
        g0c_pool.append(g0c)
    

random.sample(g0c_pool,10)


plt.figure()

for g0c in random.sample(g0c_pool,15):
    ...
    plt.plot(tout/3600,xout,linewidth=2.5)
...
for g1c in range(len(demo_g1.keys())):
    
    tout = demo_g1[str(g1c+1)]['output']['tout']   
    xout = demo_g1[str(g1c+1)]['output']['xoutS'][:,species_all.index('Mb')]
...
for g1c in random.sample(g1c_pool,15):
...
#%% test - list g1 cells by the total number of descendants
...
plt.hist(trame1_1.get_g1desc())
plt.show()

trame1_0 = drs_dict(output_main,'trame',1,0)

plt.hist(trame1_0.get_g1desc())
plt.show()


trame2_5 = drs_dict(output_main,'trame',2,5)

plt.hist(trame2_5.get_g1desc())
plt.ylabel('g1 cells')
plt.xlabel('# of descendants') 
plt.show()
...
for dl in range(10):
    ...
    drs_dict_temp = drs_dict(output_main,drug_current,rep,dl)
    
    drs_popdyn = drs_dict_temp.pop_dyn()
    
    drs_cellpop = drs_popdyn[0]
    drs_timepoints = drs_popdyn[1]
    drs_tdeaths = drs_popdyn[2][~np.isnan(drs_popdyn[2])]
    tdeaths_actual = drs_tdeaths[drs_tdeaths<=exp_time_h*3600]
    dead_count = len(tdeaths_actual)
    tp_end = np.where(drs_timepoints<=exp_time_h*3600)[0][-1]
    alive_count = drs_cellpop[tp_end]
    
    dead_count_all.append(dead_count)
    alive_count_all.append(alive_count)
    
#%%

plt.scatter(doses_all,np.array(alive_count_all)/np.array(dead_count_all))
plt.xscale('log')
plt.ylabel('# alive cells / # dead cells')
plt.xlabel('Doses (uM)')
plt.show()

#%% stratify/analyze faster dividing cells

trame2_5_g1desc = trame2_5.get_g1desc()

grp0 = np.where(trame2_5_g1desc == 0)[0] + 1
grp1 = np.where(np.logical_and(trame2_5_g1desc > 0,trame2_5_g1desc < 15))[0] + 1
grp2 = np.where(trame2_5_g1desc >= 15)[0] + 1

#%%

def get_icMedian(g1cxs):
    
    
    ics_array = np.array([trame2_5.results['output_g1'][str(g1cx)]['output']['xoutS'][0] for g1cx in g1cxs])
    
    ic_median = np.array([np.nanmedian(ics_array[:,sp]) for sp in range(np.shape(ics_array)[1])])
    
    return ic_median
    
median_ICcheck = pd.DataFrame(index=species_all)
median_ICcheck['grp0'] = get_icMedian(grp0)
median_ICcheck['grp1'] = get_icMedian(grp1)
median_ICcheck['grp2'] = get_icMedian(grp2)

median_ICcheck.to_csv(os.path.join(wd,'median_ICs.txt'),sep='\t',header=True,index=True)


#%% visualize obs trajectory for stratified cells
...
VxPARCDL = Species_doc['compartment'].values
VxPARCDL = [float(cell_params.loc[VxPARCDL[i],'volume']) for i in range(len(VxPARCDL))]


def get_observables(xout, VxPARCDL, Vc ,ObsMat):    
    Obs = []  
    Vr = VxPARCDL/Vc
...
def get_obs_gncn(gn,cn,dose_dict):
    
    xoutS_gncn = dose_dict.results['output_g'+str(gn)][str(cn)]['output']['xoutS']
    
    obs_gncn = [get_observables(xoutS_gncn[tp], VxPARCDL, Vc, ObsMat) for tp in range(len(xoutS_gncn))]
    
    return obs_gncn



def get_obs_g1grp(g1grp):
    
    obs_g1grp = {}

    for g1cx in g1grp:
    
        xoutS_g1cx = trame2_5.results['output_g1'][str(g1cx)]['output']['xoutS']
    
        obs_g1cx = [get_observables(xoutS_g1cx[tp], VxPARCDL, Vc, ObsMat) for tp in range(len(xoutS_g1cx))]
        
        obs_g1grp[str(g1cx)] = obs_g1cx
        
    return obs_g1grp

obs_grp0 = get_obs_g1grp(grp0)

obs_grp1 = get_obs_g1grp(grp1)

obs_grp2 = get_obs_g1grp(grp2)

#%%

output_temp = os.path.join(wd,'output_temp')

for obs_idx in range(len(ObsMat.columns)):
    
    plt.figure()
    

    for g1cx in grp0:
        tout = trame2_5.results['output_g1'][str(g1cx)]['output']['tout']/3600
        obs = np.array(obs_grp0[str(g1cx)])[:,obs_idx]
        plt.plot(tout,obs,color='red',label='grp0')
        
        # desc_all = trame2_5.get_desc(g1cx)
        ...
        #         for cx in range(len(gncxs)):
        #             gncx = cx+1
        #             tout = trame2_5.results['output_g'+str(gen)][str(gncx)]['output']['tout']/3600
        #             obs_gncn = get_obs_gncn(gen,gncx,trame2_5)
        #             plt.plot(tout,obs_gncn,color='green')
    ...
    for g1cx in grp1:
        tout = trame2_5.results['output_g1'][str(g1cx)]['output']['tout']/3600
        obs = np.array(obs_grp1[str(g1cx)])[:,obs_idx]
        plt.plot(tout,obs,color='green',label='grp1')
    ...
    for g1cx in grp2:
        ...
        #             tout = trame2_5.results['output_g'+str(gen)][str(gncx)]['output']['tout']/3600
        #             obs_gncn = get_obs_gncn(gen,gncx,trame2_5)
        #             plt.plot(tout,obs_gncn,color='blue')        
            

    
    plt.ylabel(str(ObsMat.columns[obs_idx]))
    plt.xlabel('Time (h)')
    # plt.legend()
    
    plt.savefig(os.path.join(output_temp,str(ObsMat.columns[obs_idx])+'.png'))
...
plt.hist(srf_trame['0']['auc'].values)
plt.show()
...
drs_dict_test.lin_tree_solo(39)
...
# plt.plot(touts_lin/3600,interpolated_values[:,list(species_all).index(sp_lin)])
...
xx = np.concatenate(xout_lin,axis=0)

xt = np.concatenate(tout_lin,axis=0)
        ...
        if len(gncxs)>0:
            for cx in range(len(gncxs)):
                ...
                if len(desc_gncx) == 0:
                    lin_gncx = drs_dict_test.results['output_g'+str(gen)][str(gncx)]['output']['lin']
                    lin_gncx = lin_gncx+'c'+str(gncx)
                    term_lin.append(lin_gncx)


#%% time average diff in obs levels




obs_diff = pd.DataFrame(index=ObsMat.columns[28:],columns=['grp0','grp2','ratio'])

for obs_idx in range(28,len(ObsMat.columns)):
    ...
    for g1cx in grp0:
        tout = trame2_5.results['output_g1'][str(g1cx)]['output']['tout']/3600
        obs = np.array(obs_grp0[str(g1cx)])[:,obs_idx]
        obs_avg = np.average(obs)
        obsgrp0.append(obs_avg)
        
    obsgrp0 = np.array(obsgrp0)
    
    obs_diff.loc[ObsMat.columns[obs_idx],'grp0'] = np.average(obsgrp0)
    ...
    for g1cx in grp2:
        ...
        obs = np.array(obs_grp2[str(g1cx)])[:,obs_idx]
        obs_avg = np.average(obs)
        obsgrp2.append(obs_avg)

        
    obsgrp2 = np.array(obsgrp2)
    
    obs_diff.loc[ObsMat.columns[obs_idx],'grp2'] = np.average(obsgrp2)
    
    obs_diff.loc[ObsMat.columns[obs_idx],'ratio'] = np.average(obsgrp2)/np.average(obsgrp0)
...
#%% time average diff in obs levels / any dose or replicate
# dose_dict = drs_dict(output_main,'trame',7,5)

dose_dict = drs_dict(output_main,'trame',9,5)


def get_obs_g1grp(g1grp,dose_dict):
    
    obs_g1grp = {}

    for g1cx in g1grp:
    
        xoutS_g1cx = dose_dict.results['output_g1'][str(g1cx)]['output']['xoutS']
...
dose_dict_g1desc = dose_dict.get_g1desc()

grp0 = np.where(dose_dict_g1desc == 0)[0] + 1
grp1 = np.where(np.logical_and(dose_dict_g1desc > 0,dose_dict_g1desc < 15))[0] + 1
grp2 = np.where(dose_dict_g1desc >= 15)[0] + 1


obs_grp0 = get_obs_g1grp(grp0,dose_dict)

obs_grp1 = get_obs_g1grp(grp1,dose_dict)

obs_grp2 = get_obs_g1grp(grp2,dose_dict)



obs_diff_n = pd.DataFrame(index=ObsMat.columns[28:],columns=['grp0','grp2','ratio'])
...
for obs_idx in range(28,len(ObsMat.columns)):
    ...
    for g1cx in grp0:
        tout = dose_dict.results['output_g1'][str(g1cx)]['output']['tout']/3600
        obs = np.array(obs_grp0[str(g1cx)])[:,obs_idx]
        obs_avg = np.average(obs)
        obsgrp0.append(obs_avg)
    ...
    obs_diff_n.loc[ObsMat.columns[obs_idx],'grp0'] = np.average(obsgrp0)
    ...
    obs_diff_n.loc[ObsMat.columns[obs_idx],'grp2'] = np.average(obsgrp2)
    
    obs_diff_n.loc[ObsMat.columns[obs_idx],'ratio'] = np.average(obsgrp2)/np.average(obsgrp0)
...
obs_idx = list(ObsMat.columns).index('p53')
...
for dl in range(10):
    ...
    for rep in range(10):
        dict0 = drs_dict(output_drs3,drug,rep+1,dl)
        dict0.dendro()

...
from utils.data_handling import load_configuration_file, load_input_data_file
from utils.files_handling import append_subfolder, check_path_existence
...
class InvalidModelName(ValueError):
    ...
    def __str__(self):
        return (
            "SPARCED ERROR: Invalid model name.\n"
...
class Model:
    ...
    def __init__(
        ...
        self.data_location = append_subfolder(
            self.path, self.configuration[const.YAML_DATA_LOCATION]
        )
        # Compilation
        self.compilation_config = self.configuration[
            const.YAML_COMPILATION_KEYWORD
        ]
        ...
        if const.YAML_SIMULATION_KEYWORD in self.configuration:
            self.simulation_config = self.configuration[
                const.YAML_SIMULATION_KEYWORD
            ]
        ...
        if const.YAML_EXPERIMENT_KEYWORD in self.configuration:
            self.experiments_config = self.configuration[
                const.YAML_EXPERIMENT_KEYWORD
            ]
            ...
            if const.YAML_EXPERIMENT_NAME in self.experiments_config:
                self.experiment_name = self.experiments_config[
                    const.YAML_EXPERIMENT_NAME
                ]
    ...
    def load_files_path(
        self, path: str | os.PathLike, files: dict[str, str]
